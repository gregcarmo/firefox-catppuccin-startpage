class Crypto extends Component {
  ...
  constructor() {
    ...
    this.refreshInterval = CONFIG.crypto?.refreshIn || this.defaults.refreshInterval;
  }
  ...
  imports() {
    return [this.resources.icons.cryptofont, this.resources.icons.material, this.resources.fonts.roboto];
  }
  ...
  setCryptoIcon() {
    ...
    icon.className = icon.className.replace(cryptoFontMatcher, iconClassName);
  }
  ...
  get coin() {
    return this.refs.cryptoCoin.innerText;
  }
  ...
  async getCurrentPrice() {
    const exchangeRate = await this.exchangeClient.getExchangeRate(this.coin, this.currency);
  ...
  async connectedCallback() {
    ...
    setInterval(async () => this.setExchangeRate(), REFRESH_IN_SECS);
  }
  ...
  style() {
    return `
      :host {
          --main-accent: ${CONFIG.palette.green};
      }
  ...
  async template() {
    return `
        ...
        <div class="crypto-value crypto-icon">
            <span class="crypto-coin">${this.defaults.coin}</span>
